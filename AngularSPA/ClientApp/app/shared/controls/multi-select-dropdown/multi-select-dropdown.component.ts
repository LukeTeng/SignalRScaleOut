...
export class MultiselectDropdown implements OnInit, DoCheck, ControlValueAccessor, Validator {
  ...
  selectOptionsValue(id: number) {
    return this.options.filter(p => p.id === id)[0].name;
  }

  removeChoice(event: Event, ichoice: number) {
    event.stopPropagation();
    this.model = this.model.filter(p => p !== ichoice);
    this.title = this.options
      .filter((option: IMultiSelectOption) =>
        this.model && this.model.indexOf(option.id) > -1
      )
